...
public static class Recursion
{
    ...
    {
        // 1. Initialize the path if this is the first call
        if (currPath == null)
        {
            currPath = new List<ValueTuple<int, int>>();
        }

        // 2. Validate the move using the specific method from Maze.cs
        // Note: IsValidMove checks bounds, walls (0), and circles (currPath.Contains)
        if (!maze.IsValidMove(currPath, x, y))
        {
            return;
        }

        // 3. Add the current valid position to the path
        currPath.Add((x, y));

        // 4. Check if we have reached the end square (value 2)
        if (maze.IsEnd(x, y))
        {
            // Add the formatted string representation of the path to results
            results.Add(currPath.AsString());
        }
        else
        {
            // 5. Recursive Step: Explore all four cardinal directions.
            // We create a NEW list for each branch to ensure backtracking works correctly.
